...
#### Audio Output Device Switching (WebRTC)
- **Fixed all audio output device switching** - The audio session category is now properly reconfigured for ALL device types, matching the pattern in `AudioPlayback.swift`. Previously, Bluetooth, wired headphones, system default, and smart mode only called `overrideOutputAudioPort(.none)` without clearing the `.defaultToSpeaker` option from the initial configuration. Now:
  - **Speaker**: Reconfigures with `.defaultToSpeaker` and overrides to speaker
  - **Receiver**: Reconfigures without `.defaultToSpeaker`, defaults to receiver
  - **Bluetooth/Headphones/System Default**: Reconfigures without `.defaultToSpeaker`, allows system routing
  - **Smart**: Dynamically adds `.defaultToSpeaker` only when no Bluetooth is connected
